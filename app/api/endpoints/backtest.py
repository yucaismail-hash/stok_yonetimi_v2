...
def run_async_backtest_job(task_id: str, user_id: int, request: BacktestBatchRequest, db: Session):
    ...
    try:
        ...
        for idx, material in enumerate(materials):
            try:
                ...
                print(f"❌ Backtest hatası ({material.get('code', '')}): {e}")
                continue
        ...
        # ✅ Tek bir kayıt olarak kaydet (tüm sonuçları içinde)
        analysis_result = UserAnalysisResult(
            user_id=user_id,
            result_type='backtest_batch',
            material_code='BATCH_ALL',  # Toplu kayıt için
            material_group='TOPLU',
            result_data={
                'total': len(results),
                'results': results,  # ✅ Tüm sonuçlar burada
                'test_window': test_window,
                'strategies_tested': strategies_to_test
            },
            params={
