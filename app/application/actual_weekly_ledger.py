...
class ActualWeeklyLedgerService:
    ...
    def _decide(self, company_id, revision_id, actor_user_id, approved):
        ...
        try:
            ...
            revision.approval_status = "accepted" if approved else "rejected"; revision.approved_by_user_id = actor_user_id
            if approved:
                revision.approved_at = datetime.now(timezone.utc); observation = session.query(ActualWeeklyObservation).filter_by(id=revision.observation_id, company_id=company_id).one()
            ...
            else: revision.rejected_at = datetime.now(timezone.utc)
            session.commit(); return str(revision.id)
        except Exception:
            session.rollback(); raise
        finally: session.close()
