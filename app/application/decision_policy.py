...
class DecisionPolicy:
    ...
    def evaluate(self, envelope):
        ...
        support=[]; conflicts=[]; candidates=[]
        def add(kind,severity,priority,reason,evidence,change):
            candidates.append(DecisionCandidate(kind,severity,priority,envelope.decision_context,(reason,),tuple(evidence),(),0.0,(),(change,)))
        ...
        if any(c.candidate_type=="REVIEW_FORECAST" for c in candidates) and optional.get("backtest",{}).get("signal")=="weak_validation":
        ...
        candidates=[DecisionCandidate(c.candidate_type,c.severity,c.priority,c.decision_context,c.reason_codes,c.supporting_evidence,c.conflicting_evidence,confidence,c.expected_impact_references,c.what_would_change_this) for c in candidates]
