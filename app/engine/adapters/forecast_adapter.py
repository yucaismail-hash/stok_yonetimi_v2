...
def forecast_adapter(implementation, prepared_input, request, resolver_factory=ChampionResolver, inference_factory=XGBoostChampionInferenceService, event_resolver_factory=EventIntelligenceResolver):
    ...
    event_resolver=event_resolver_factory() if isinstance(cutoff,str) and isinstance(demand_type,str) else None
    started = perf_counter(); items = []
    for item in prepared_input["items"]:
        code=item["material_code"]; resolution=None
        event_context=event_resolver.resolve(request.company_id,code,demand_type,cutoff) if event_resolver else {'status':'EVENT_INTELLIGENCE_ABSENT','memories':[]}
        ...
        if resolution is not None and resolution.kind=='XGBOOST_ARTIFACT':
            ...
            info={'champion_resolution':'xgboost_artifact','champion_registry_entry_id':str(prediction.champion_registry_entry_id),'model_artifact_id':str(prediction.model_artifact_id),'artifact_checksum':prediction.artifact_checksum,'feature_schema_version':prediction.feature_schema_version,'inference_strategy_version':prediction.inference_strategy_version,'training_cutoff_period':prediction.training_cutoff_period,'forecast_cutoff_period':cutoff,'demand_type':demand_type,'event_intelligence':event_context}
        ...
        info['event_intelligence']=event_context
