...
def simulation_adapter(implementation, prepared, request):
    ...
    sim=implementation(n_simulations=n); started=perf_counter(); rows=[]
    for item in prepared['items']:
        ...
        output=sim.simulate(float(initial),lead,lead_std,demand_mean,demand_std,float(eoq),float(rop),weeks=weeks,use_regime=bool(p.get('use_regime',False)),historical_demand=None,use_copula=bool(p.get('use_copula',False)),use_adaptive_ss=bool(p.get('use_adaptive_ss',False)))
        rows.append({'material_code':item['material_code'],'service_level':output['service_level'],'stockout_probability':output['stockout_probability'],'avg_stock':output['avg_stock'],'cvar_95':output['cvar_95'],'weeks':weeks,'n_simulations':n,'regime_used':output['regime_used'],'copula_used':output['copula_used'],'adaptive_ss_used':output['adaptive_ss_used'],'initial_stock':initial,'lead_time_days':lead,'lead_time_std_days':lead_std,'eoq':eoq,'rop':rop,'forecast_source':forecast_source,'safety_stock_source':safety_stock_source,'rop_source':rop_source,'supplier_enrichment':item.get('supplier_enrichment'),'provenance':item.get('provenance')})
