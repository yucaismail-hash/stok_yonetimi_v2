...
class RecommendationEngine:
    ...
    def get_top_recommendation(self, dashboard_summary: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Args:
            dashboard_summary: DashboardSummaryEngine çıktısı
        
        Returns:
            {
                "analysis": "forecast",
                "priority": 95,
                "title": "Update Forecast Analysis",
                "reason": "Demand trend has changed.",
                "expected_benefit": "Safety Stock and purchasing decisions will be recalculated.",
        ...
        # Priority'si olan modülleri bul
        active_modules = []
    ...
    def _generate_reason(self, module: str, data: Dict[str, Any]) -> str:
        """Modüle göre reason oluştur."""
        reasons = {
            'forecast': f"Son analiz {data.get('created_at', '')} tarihinde yapıldı. Talep trendi değişmiş olabilir.",
            ...
            'simulation': f"{data.get('total_items', 0)} ürün simüle edildi. İyileştirme fırsatları var.",
            'backtest': f"{data.get('total_items', 0)} ürün test edildi. Performans değerlendirilmeli.",
        }
    ...
    def _generate_expected_benefit(self, module: str, data: Dict[str, Any]) -> str:
        """Modüle göre expected benefit oluştur."""
        benefits = {
