...
async def main():
 ...
 try:
  ...
  if s:
   s.rollback()
   if c:
    eids=[x[0] for x in s.query(RuntimeExecution.execution_id).filter_by(company_id=c.id).all()]; vids=[x[0] for x in s.query(ForecastVintage.id).filter_by(company_id=c.id).all()]; sids=[x[0] for x in s.query(DecisionSnapshot.id).filter_by(company_id=c.id).all()];s.query(DecisionSnapshotCandidate).filter(DecisionSnapshotCandidate.decision_snapshot_id.in_(sids)).delete(synchronize_session=False);s.query(DecisionSnapshot).filter_by(company_id=c.id).delete(synchronize_session=False);s.query(ForecastVintagePoint).filter(ForecastVintagePoint.forecast_vintage_id.in_(vids)).delete(synchronize_session=False);s.query(ForecastVintage).filter_by(company_id=c.id).delete(synchronize_session=False);s.query(RuntimeResultReference).filter(RuntimeResultReference.execution_id.in_(eids)).delete(synchronize_session=False);s.query(RuntimeTaskAttempt).filter(RuntimeTaskAttempt.execution_id.in_(eids)).delete(synchronize_session=False);s.query(RuntimeTask).filter(RuntimeTask.execution_id.in_(eids)).delete(synchronize_session=False);s.query(RuntimeExecution).filter(RuntimeExecution.execution_id.in_(eids)).delete(synchronize_session=False);s.query(ActualWeeklyRevision).filter_by(company_id=c.id).delete(synchronize_session=False);s.query(ActualWeeklyObservation).filter_by(company_id=c.id).delete(synchronize_session=False);s.query(Dataset).filter_by(id=d.id).delete(synchronize_session=False);s.query(CompanyEncryptionKey).filter_by(user_id=u.id).delete(synchronize_session=False);s.query(User).filter_by(id=u.id).delete(synchronize_session=False);s.query(Company).filter_by(id=c.id).delete(synchronize_session=False);s.commit();s.close()
